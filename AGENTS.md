...
This repo's `browserslist` was `[">0.2%", "not dead", "not ie <= 11", "not op_mini all"]` — missing the `Chrome >= 45`/`Android >= 4.4` entries present in the reference boilerplate. `vite.config.js`'s `build.minify` was already correctly set to `'terser'` with the explanatory comment, so that half was fine. Found during a full fleet audit and fixed by restoring the two missing browserslist entries; after the fix, `npx browserslist` resolves down to `android 4.4` / `chrome 45`, and the rebuilt `build/assets/index-legacy-*.js` has zero untranspiled arrow functions and no real `?.`/`??` leakage.
